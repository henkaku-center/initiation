...
```jsonc
{
  ...
  "ppq": 480,
  "lengthBars": 33,          // 素材の長さ。最後のノートが終わる小節まで
  "loopBars": 33,            // ループの折り返し位置。既定は lengthBars と同じ
  "loop": { "start": "0:0:0", "end": "33:0:0" },
  "tracks": [
    {
      "name": "drums",
      "sourceName": "808",
...
- 134 BPM / 4/4 / 480 ppq。`ePiano` は4小節のクリップを8個(0, 4, 8, 12, 16, 20, 24, 28小節目)
  並べたもので、素材全体は33小節。
...
- 8クリップのうち最初の6個(0, 4, 8, 12, 16, 20小節目)には **9音の余りが含まれる。** クリップの
  終わりを越えた位置に E4 G4 B4 が3打あり、1打目はちょうど次のクリップの頭と同じ tick、残り2打は
  そこから +183〜186 tick と +437〜440 tick。6クリップすべてで同じ形なので、演奏ではなくクリップ
  ごと複製された残骸(**書き出し元でリージョンを4小節で切っていたため鳴らず、MIDI書き出しで
  クリップの中身がそのまま出たものと考えられる**)。24小節目・28小節目のクリップにはこの残骸がない。
- 1本にまとめるとこの残骸が次のクリップの頭(A4 D4 F4)と重なり、6音のクラスタになって和音が濁る
  (7.16 / 14.33 / 21.49 / 28.66 / 35.82 / 42.99 秒、残骸のある6クリップぶん)。
  **`--trim-clip-overlap` で54音(9音 x 6クリップ)を削る。** はみ出しがあるのは `ePiano` だけで、
  他の3トラックは変わらない。
- 残骸を削っても素材の長さは変わらない(33小節のまま)。最後に鳴り終わるのは `drums` のトラックで、
  `ePiano` の残骸が伸ばしていたわけではないため。
- `ePiano` `bass` `lead` は(残骸を削れば)量子化なしで16分グリッドに完全に乗る。
  グリッド外に残るのは `drums` の36音だけで、これはキット側の意図的なずれ。
- 捨てたイベント: Program Change は0件、Control Change も0件。音源固有の設定は含まれていない。
- `drums` は元の演奏の揺れを残している(16分グリッドから最大 45 tick ずれる)。機械的に揃えたい
  場合は `--quantize 1/16` を付けて再生成する。
