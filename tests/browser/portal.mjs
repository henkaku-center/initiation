...
try {
  ...
  await page.locator("dialog").waitFor({ state: "hidden" });
  assert.equal(await home.locator("#frequency-title").count(), 1);
...
await page.getByLabel("あなたを、なんとお呼びしたらいいですか？").fill("Local traveller");
await page.getByRole("button", { name: "NEXT →", exact: true }).click();
await textPresent("HENKAKUで、気になることは？");
await page.getByRole("button", { name: "MUSIC", exact: true }).click();
let saveAction;
page.on("request", (request) => {
  if (new URL(request.url()).pathname === "/initiation" && request.method() === "POST") saveAction = { url: request.url(), id: request.headers()["next-action"], body: request.postData() };
});
await failedAction("/initiation", page.getByRole("button", { name: "NEXT →", exact: true }), "保存できませんでした");
assert.equal(await page.getByRole("button", { name: "MUSIC ↗", exact: true }).getAttribute("aria-pressed"), "true");
// Opening Initiation is recorded on its own row, apart from the questions (Issue #120).
const questionRows = (entries) => entries.filter((entry) => entry.step_id !== "v2-opened");
const openingRows = (entries) => entries.filter((entry) => entry.step_id === "v2-opened");
let progress = await rows("initiation_progress", idA);
assert.equal(questionRows(progress).length, 0, "A failed answer save leaves no question rows");
assert.equal(openingRows(progress).length, 1, "Opening Initiation is recorded even when the answer save fails");
assert.deepEqual(JSON.parse(openingRows(progress)[0].answer), { status: "seen" });
await page.getByRole("button", { name: "NEXT →", exact: true }).click();
await page.getByLabel("いま、気になっていることを教えてください。").waitFor();
const introductionAction = { ...saveAction };
await page.reload();
await page.getByLabel("いま、気になっていることを教えてください。").waitFor();
progress = await rows("initiation_progress", idA);
assert.equal(questionRows(progress).length, 1);
assert.equal(openingRows(progress).length, 1, "Reloading does not record the opening again");
await screenshot("journey-desktop");
await screenshot("journey-tablet", 768);
await screenshot("journey-mobile-dark", 360, "dark");
await page.getByLabel("いま、気になっていることを教えてください。").fill("Aの好奇心：音楽を一緒につくる");
await page.getByRole("button", { name: "NEXT →", exact: true }).click();
await page.getByLabel("誰かと一緒に、何かを動かした経験はありますか？").waitFor();
await page.getByRole("button", { name: "まだ言葉にしない", exact: true }).click();
await page.getByRole("button", { name: "まだ迷っている", exact: true }).click();
await page.getByRole("button", { name: "NEXT →", exact: true }).click();
await page.getByRole("button", { name: "CURIOSITY", exact: true }).click();
await page.getByRole("button", { name: "旅を終える →", exact: true }).click();
await textPresent("旅の記録を保存しました");
progress = await rows("initiation_progress", idA);
assert.deepEqual(questionRows(progress).map((entry) => entry.step_id).sort(), ["v2-contribution", "v2-curiosity", "v2-experience", "v2-interests", "v2-readiness"]);
assert.equal(questionRows(progress).filter((entry) => JSON.parse(entry.answer).status === "skipped").length, 1);
assert.equal(openingRows(progress).length, 1);
// Re-edit the adopted questions, keeping prior values and recording a deliberate clear.
await page.getByRole("button", { name: "回答を見直す", exact: true }).click();
assert.equal(await page.getByLabel("あなたを、なんとお呼びしたらいいですか？").inputValue(), "Local traveller");
await page.getByRole("button", { name: "NEXT →", exact: true }).click();
assert.equal(await page.getByRole("button", { name: "MUSIC ↗", exact: true }).getAttribute("aria-pressed"), "true");
await page.getByRole("button", { name: "NEXT →", exact: true }).click();
assert.equal(await page.getByLabel("いま、気になっていることを教えてください。").inputValue(), "Aの好奇心：音楽を一緒につくる");
await page.getByLabel("いま、気になっていることを教えてください。").fill("Aの好奇心：再編集した音楽");
await page.getByRole("button", { name: "NEXT →", exact: true }).click();
await page.getByLabel("誰かと一緒に、何かを動かした経験はありますか？").waitFor();
await page.getByRole("button", { name: "まだ言葉にしない", exact: true }).click();
await page.getByRole("button", { name: "まだ言葉にしない", exact: true }).click();
...
await textPresent("旅の記録を保存しました");
progress = await rows("initiation_progress", idA);
assert.equal(questionRows(progress).length, 5, "Re-answer updates records without duplicates");
assert.equal(openingRows(progress).length, 1, "Re-editing keeps a single opening record");
assert.equal(JSON.parse(progress.find((entry) => entry.step_id === "v2-curiosity").answer).value, "Aの好奇心：再編集した音楽");
assert.equal(JSON.parse(progress.find((entry) => entry.step_id === "v2-readiness").answer).status, "skipped");
await page.getByRole("link", { name: "あなたのパスポートへ ↗", exact: true }).click();
await page.getByRole("link", { name: "回答を見直す →", exact: true }).click();
await page.waitForURL(`${origin}/initiation?edit=1`);
await page.getByLabel("あなたを、なんとお呼びしたらいいですか？").waitFor();
await page.getByRole("link", { name: "My passport ↗", exact: true }).click();
  let applyAction;
  page.on("request", (request) => {
  ...
  await textPresent("審査待ち");
  console.log("PASS journey/passport: failure recovery, reload/resume, five versioned server entries and re-answer persistence, application and duplicate prevention");
  ...
  await textPresent("まずは、小さな旅に出よう。");
...
const retiredStep = await replay(introductionAction, JSON.stringify(["q-introduction", "old shortcut"]));
assert.match(retiredStep.text, /不明なステップ/);
await replay(introductionAction, JSON.stringify(["v2-curiosity", { status: "answered", value: "Bの好奇心" }, { memberId: idA }]));
  assert.equal(JSON.parse((await rows("initiation_progress", idB)).find((entry) => entry.step_id === "v2-curiosity").answer).value, "Bの好奇心");
  assert.equal(JSON.parse((await rows("initiation_progress", idA)).find((entry) => entry.step_id === "v2-curiosity").answer).value, "Aの好奇心：再編集した音楽");
  const admin = await page.goto(`${origin}/admin`);
  assert.equal(admin.status(), 404);
  await page.goto(`${origin}/initiation`);
  await textPresent("HENKAKUで、気になることは？");
  assert.equal(await page.getByText("Aの好奇心：再編集した音楽").count(), 0);
