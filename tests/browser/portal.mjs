...
assert.deepEqual(progress.map((entry) => entry.step_id).sort(), ["v2-contribution", "v2-curiosity", "v2-experience", "v2-interests", "v2-readiness"]);
assert.equal(progress.filter((entry) => JSON.parse(entry.answer).status === "skipped").length, 1);
...
assert.equal(progress.length, 5, "Re-answer updates records without duplicates");
...
await replay(introductionAction, JSON.stringify(["v2-curiosity", { status: "answered", value: "Bの好奇心" }, { memberId: idA }]));
  assert.equal(JSON.parse((await rows("initiation_progress", idB))[0].answer).value, "Bの好奇心");
