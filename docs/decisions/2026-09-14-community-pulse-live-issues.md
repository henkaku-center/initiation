...
[固定4件を手動で掲載する決定](./2026-09-09-community-pulse-open-issues.md)を置き換え、掲載対象の管理を固定リストからラベルへ移す。

- 本家 `henkaku-center/initiation` の `community-pulse` ラベル付きopen Issueを更新日時の降順で最大6件表示する。ラベルの大文字小文字は区別しない。PRは除外し、固定のIssue API URLに `page=1..5` を付けて100件ずつ取得する。6件に達するか応答が100件未満なら終了し、100件ちょうどで6件未満なら次のページを確認する。GitHubのLinkヘッダーのURLは使用しない。同時刻は番号の降順。同時更新による重複は番号で除く。
...
- 再試行抑止中もstaleなスナップショットへのアクセスごとにNextのバックグラウンド再検証は起動し、即座に失敗するため、Nextの再検証エラーログが出る。GitHubへの通信は抑止され、前回成功分と取得時刻は保持されるが、障害が長引くとアクセス数に応じてログが増える。
- 公開APIは `Cache-Control: no-store` を返し、応答やブラウザには追加キャッシュを設けない。鮮度の判定を毎回行い、キャッシュを二重にして更新を遅らせない。クライアントのfetchは既定のキャッシュ設定を使う。リクエストに `no-cache` が付くと `next dev` のData Cacheが無効になるため、クライアント側で `cache: "no-store"` を指定しない。Homeは表示時に一度取得し、ポーリングやタブ復帰による再取得は行わない。
- Issueタイトルはテキストとして描画する。リンクは本家のIssue URLに限定し、本文・投稿者情報・トークン・上流のエラー本文はクライアントへ渡さない。
- GitHub一覧へのリンクは生成HTMLに置き、取得失敗時にも利用できるようにする。クライアントが使わない `sourceUrl` はAPI応答へ含めない。

通常ポータルへの接続・デモ撤去はPR #105で完了しているため、現行の `assets/reference/gateway/` と `scripts/gateway/` を編集し、生成物も揃える。#52・#91の完了条件は変更しない。ラベル作成・掲載対象への付与は運営が行う。
