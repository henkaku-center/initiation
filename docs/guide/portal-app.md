...
| Community | `/community`、従来の`/checkin` | 本人の日次チェックイン、その日の一言(Signal)、履歴 |
| Passport | `/passport`、従来の`/apply` | 完走状態、Discord名の登録、申請・再申請、審査・Allowlist・配布の状態 |
...
チェックインには任意の一言(Signal)を添えられます。書いてから押すことも、押してから書き足すこともでき、その日の一言は上書きされて最新のものが残ります。1日1件の制約は変わりません。空にして送ると一言を消せます。長さは140文字までで、一言の保存はチェックインとは別のレート制限の枠で数えます(押した直後に書き足して弾かれないようにするため)。一言は本人の履歴にだけ表示し、他のメンバーへは公開しません。

Discord名は申請の前提として `/passport` で受け取ります。問いへの回答ではないため、Initiationの質問の並びには置きません。完走後に入力欄が出て、未登録のまま申請しようとすると申請は作られません。アプリはDiscordへ問い合わせず、実在もユーザー名の一致も確認しません。承認時に運営が `/admin` の申請一覧で突き合わせます。扱いは[プライバシーポリシー](../privacy-policy.md)に従います。
