...
7. Firewallのレート制限ルールを1本作り、`/api/` 以下をIP単位で60秒あたり100リクエストに制限します。最初は動作を Log にして観測し、分布を見てから Deny（429）へ切り替えます（[未認証エンドポイントのレート制限](../decisions/2026-09-20-unauthenticated-rate-limits.md)）。
