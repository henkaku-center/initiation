...
Issue #91の状態表示、指定されたLiquid/Gatewayの入口、Game Aの5ステージを組み合わせた独自ブランチのデモです。トップページは参照元のHTML/CSS/JSを基準にし、同一オリジンの文書内で元のスクロール構造を維持しています。上流コミュニティの本番仕様を確定するものではありません。
...
1. トップの解読・泡・調整タブを見る。調整タブには解読19項目、色、Bubble 12項目、イントロ再生があります。
...
- Liquidの元コードに基づく解読・ロール・泡と調整パネル。添付の黒いマークをSVG化しています。
