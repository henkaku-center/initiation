...
12. Homeの先頭とイントロ表示中はYouTube API／プレーヤーをまだ読み込まず、Voicesに近づいた時に読み込むことを確認する。初期停止を確認してから「映像を無音で再開」を押す。縦スクロールでカードが左右端まで流れ、横長画面でも先頭から最後へ再生対象が1本ずつ切り替わることを確認します。再読み込みや別画面からホームに戻ると停止状態になります。
13. Frequencyに「みんなの最近のプレイ履歴」、抽象アイコン3種と匿名アイコン、4件の架空の再生イベントがあることを確認する。再読み込みしても共通のサンプルが残り、各リンクから公式動画へ進めます。Pulseの4件は本家Issueへの直接リンクで、開発中の注記と確認日を表示します。
...
- Frequencyはコミュニティ全体の最近のプレイ履歴を4件の共通モックで表示します。リスナーは抽象アイコン3種と匿名アイコンで表し、識別子・再生時刻は架空です。個人の進捗・続きから再生・ブラウザごとの履歴保存は行いません。実際のユーザー識別やYouTube視聴履歴との連携もありません。
...
- 配色は `henkaku.appearance.v1` に保存します。デモ進捗とは独立した設定で、「デモ操作」の進捗リセットでは消しません。保存不能なら、その表示中だけの設定として動作します。Frequencyは共通の固定サンプルを使い、旧キー `henkaku.podcast-demo.history.v1` は読み書きしません。
