...
`nonce` はサーバーが発行し、暗号化したCookieの中にだけ保存します。**サーバー側に nonce を記録するテーブルはありません。** 検証の成功・失敗にかかわらず、試行後にCookieから消しますが、これが効くのは新しいCookieを受け入れるクライアントだけです。同じ署名の再利用を実際に抑えているのは次の2つです（[Issue #50](https://github.com/henkaku-center/initiation/issues/50) / [決定事項](/decisions/2026-08-24-siwe-nonce-single-use)）。

- **Cookieへの束縛** — nonce 発行時のCookieを持っている相手しか検証に入れない
- **有効期限（15分以内）** — 期限を過ぎた署名は、正しいCookieと組み合わせても通らない
