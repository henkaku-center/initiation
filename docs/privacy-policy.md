...
申請時に入力するDiscordのユーザー名は、承認後に運営が連絡するための連絡先です。本サービスはDiscordへ問い合わせず、ユーザー名の実在も確認しません。運営が現実の身元を特定するために照合するものではありませんが、**この入力によって、ウォレットアドレスとDiscordアカウントの対応が運営の申請審査担当者に見える状態になります**。Discord上で現実の身元が分かる表記を使っている場合は、その点をふまえて入力してください。
...
| メンバー情報 | ウォレットアドレス、呼び名(任意)、Discordのユーザー名(申請する場合は必須) | メンバーの識別、申請と進捗の紐付け、申請一覧での申請者の識別、承認後の連絡 |
| Initiation | ステップID、回答、完了日時 | 進捗の保存・再表示、完走判定 |
| チェックイン | チェックイン日、記録日時、本人が書いた一言(任意) | 活動記録の保存・再表示 |
...
本サービスは、シードフレーズや秘密鍵の入力を求めません。Initiationの回答とチェックインの一言にも、個人を特定できる情報や秘密情報を記載しないでください。
...
- ウォレットアドレスとDiscordのユーザー名の対応は、申請の審査と承認後の連絡に必要な運営の担当者だけが `/admin` の申請一覧で閲覧します。一般公開せず、他のメンバーへも表示しません。
- 本人は、自分の進捗、チェックイン履歴、申請状態と本人向けの判断理由を閲覧できます。
- チェックインの一言は、現在は本人の履歴にだけ表示します。他のメンバーやコミュニティへ公開する場合は、公開範囲を改めて決めてからこの方針を更新します。
