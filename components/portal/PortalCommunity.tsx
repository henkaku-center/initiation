...
export function PortalCommunity({ history, today, signedIn = true }: { history: Checkin[]; today: string; signedIn?: boolean }) {
  ...
  return <main className="pd-page">
    <section className="pd-daily-checkin" aria-labelledby="daily-checkin-title">
      ...
      <div className="pd-checkin-panel"><div><p className="pd-eyebrow">{today} / JST</p><h2>今日は、ここにいる。</h2><p>チェックインは日本時間で1日1回。一言は後から書き足したり、書き直したりできます。記録は本人用の履歴に残ります。</p>{!signedIn && <Link className="pd-text-button" href="/setup">サインインしてチェックイン ↗</Link>}</div>{signedIn ? <CheckinButton key={today} checked={history.some((entry) => entry.checkinDate === today)} note={history.find((entry) => entry.checkinDate === today)?.note ?? null} /> : <button className="pd-checkin-button" type="button" disabled><span>✳</span>CHECK IN</button>}</div>
      <section className="pd-checkin-history" aria-labelledby="footprints-title"><span className="pd-mono">YOUR FOOTPRINTS</span><h2 id="footprints-title">ここにいた、あなたの記録。</h2>{!signedIn ? <p>サインインすると、あなたの記録を確認できます。</p> : history.length ? <div className="pd-footprints">{history.map((entry) => <div key={entry.id}><span>✳</span><strong><time dateTime={entry.checkinDate}>{entry.checkinDate}</time></strong><small>{entry.note ?? "CHECKED IN"}</small></div>)}</div> : <p>まだ履歴はありません。最初のチェックインが、あなたの足あとになります。</p>}<p className="pd-fineprint">1日1回（日本時間）。サインインしたあなたの記録です。</p></section>
    </section>
