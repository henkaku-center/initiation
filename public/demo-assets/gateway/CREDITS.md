...
`bubble.js` and `decrypt-reveal.js`: Copyright (c) 2026 David Haz, MIT + Commons Clause License Condition v1.0. See `LICENSE-canvas-ui.txt`, retained in full. Used as part of this website; not offered as an independent component library. These files retain their own license rather than the repository's MIT default.

`gateway-v1-claude-decrypt.js` and the HTML/CSS: HENKAKU UI prototype source, with its source attribution retained.
...
The following credits document the earlier preview implementation preserved in Git history. The unused player code and fictional history data have been removed from the application. Current Home documents do not load the YouTube players, thumbnails or fictional community listening events.

Videos / Joi Ito and the respective rights holders / https://www.youtube.com/@joiito/podcasts → “Joi Ito's Podcast (VIDEO)”, https://www.youtube.com/playlist?list=PLdwOpF5thM-7Q5OILxkZpRnUnrt0w7US2 and the per-video URLs recorded in the earlier revision of `gateway-data.js` (checked 2026-09-09) / externally hosted YouTube content, used through the official embedded player; no CC BY 4.0 or repository MIT relicensing is claimed.
