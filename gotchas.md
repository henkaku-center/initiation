...
- 2026-09-12: デモを通常アプリへ移す作業では、模擬画面・状態ストア・ビルド分岐を移行完了後も維持する前提にしない。共有UIと素材の依存を確認して通常版へ移し、不要な経路・操作・生成物・CI・公開手順まで揃えて整理する。検証用コピーにも削除を同期し、残った旧ファイルを現在のソースとして検証しない。
